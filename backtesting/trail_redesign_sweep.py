"""
Trail-redesign sweep: 5 variants on 21-month + walk-forward winner.

Background: NO_TRAIL (binary disable) showed +Rs 2.11L 21mo lift but only
3/6 PnL wins on walk-forward (MARGINAL). Test more sophisticated variants
that keep the trail mechanism but tune its sensitivity.

Variants:
  1. BASELINE    — deployed Option B (control)
  2. NO_TRAIL    — trail entirely disabled (the prior MARGINAL candidate)
  3. DELAYED     — same trail % but longer min_hold (48 bars vs 24/12)
  4. DEEP        — same trail % but require 1% profit before trail engages
  5. DEEP_DELAY  — combined: 48 min_hold + 1% min profit
  6. CHANDELIER  — use_chandelier_exit_v15=True with chandelier_atr_mult=2.5

Phase 1: 21-month sweep, identify best variant
Phase 2: Walk-forward the winner across 6 windows

Usage:
    python -m backtesting.trail_redesign_sweep
"""
import sys
...
VARIANTS = {
    ...
    "DEEP": lambda: {**base_cfg(),
        "trail_min_profit_pct": 0.010,  # 1% min profit before trail engages
    },
    "DEEP_DELAY": lambda: {**base_cfg(),
        "min_hold_trail_put": 48, "min_hold_trail_call": 48,
        "monwed_min_hold_trail_put": 24, "monwed_min_hold_trail_call": 24,
        "trail_min_profit_pct": 0.010,
    },
    "CHANDELIER": lambda: {**base_cfg(),
        "use_chandelier_exit_v15": True,
...
def main():
    ...
    print(f"  {'variant':<12} {'n':>4} {'PnL':>14} {'PF':>5} {'WR%':>6} "
          f"{'DD':>11} {'trail_n':>8}")
    print("  " + "-" * 70)
    results_21mo = {}
    for name, builder in VARIANTS.items():
        cfg = builder()
        m, dd, trades, n_trail = run_window(cfg, full_start, full_end, preloaded)
        results_21mo[name] = (m, dd, n_trail)
        print(f"  {name:<12} {m['n']:>4d} Rs {m['pnl']:>+12,.0f} {fmt_pf(m['pf'])} "
              f"{m['wr']:>5.1f}% Rs {dd:>+9,.0f} {n_trail:>8d}", flush=True)

    # Identify best — highest PnL among non-baseline
    best_name = None
    ...
    for label, w_start, w_end in WINDOWS:
        ...
        ma, _, _, _ = run_window(cfg_a, w_start, w_end, pre)
        mx, _, _, _ = run_window(cfg_x, w_start, w_end, pre)
        d_pnl = mx["pnl"] - ma["pnl"]
